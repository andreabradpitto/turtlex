...
from gym.wrappers.monitor import load_results # CORRETTO DA ME a causa di funzione deprecata in gym (1/3))
...
class LivePlot(object):
    def __init__(self, outdir, data_key='episode_rewards', line_color='blue'):
        ...
        fig = plt.gcf().canvas.manager.set_window_title('averaged_simulation_graph') # CORRETTO DA ME a causa di funzione deprecata in gym (2/3))
        matplotlib.rcParams.update({'font.size': 15})

    def plot(self, full=True, dots=False, average=0, interpolated=0):
        #results = gym.monitoring.load_results(self.outdir)
        results = load_results(self.outdir) # CORRETTO DA ME a causa di funzione deprecata in gym (3/3))
