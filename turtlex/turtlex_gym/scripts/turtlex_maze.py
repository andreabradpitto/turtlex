...
# The path is __init__.py of openai_ros, where we import the TurtlexMazeEnv directly
timestep_limit_per_episode = 100 # Can be any Value

register(
        id='MioTurtlexMaze-v0',
        entry_point='turtlex_maze:TurtlexMazeEnv',
        max_episode_steps=timestep_limit_per_episode,
    )

class TurtlexMazeEnv(turtlex_env.TurtlexEnv):
    def __init__(self):
        """
        ...
        self.dec_obs = rospy.get_param("/turtlex/number_decimals_precision_obs", 1)
        ...
        self.n_observations = rospy.get_param('/turtlex/n_observations')
        ...
        super(TurtlexMazeEnv, self).__init__()
        ...
        self.laser_filtered_pub = rospy.Publisher('/turtlex/laser/scan_filtered', LaserScan, queue_size=1)
    ...
    def _is_done(self, observations):
        ...
        if self._episode_done:
            rospy.logdebug("Turtlex is Too Close to wall==>"+str(self._episode_done))
        else:
            rospy.logerr("Turtlex is Ok ==>")
    ...
    def discretize_observation(self,data,new_ranges):
        ...
        for i, item in enumerate(data.ranges):
            if (i%mod==0):
                ...
                # We add value zero
                filtered_range.append(0.1) # = to min_laser_value for the hokuyo sensor. Ma questo else non serve a nulla mi sa,
                                           # viene mandato in return solo il vettore da 6 "discretized_ranges". Questo else può essere tolto
