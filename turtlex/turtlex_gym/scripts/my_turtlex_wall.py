...
register(
        id='MyTurtlexWall-v0',
        entry_point='my_turtlex_wall:MyTurtlexWallEnv',
        max_episode_steps=timestep_limit_per_episode,
    )

class MyTurtlexWallEnv(turtlex_env.TurtlexEnv):
    def __init__(self):
        """
        ...
        #num_laser_readings = int(len(laser_scan.ranges)/self.new_ranges) # RIPARATO DA ME mettendo int
        ...
        super(MyTurtlexWallEnv, self).__init__()
        ...
        if self._episode_done:
            ...
            if current_position.x <= MAX_X and current_position.x > MIN_X:
                if current_position.y <= MAX_Y and current_position.y > MIN_Y:
                    ...
                    rospy.logerr("Turtlex to Far in Y Pos ==>"+str(current_position.x))
                    self._episode_done = True
            else:
                rospy.logerr("Turtlex to Far in X Pos ==>"+str(current_position.x))
                self._episode_done = True
