...
# TODO PER ORA QUESTO E IL FILE DI START LEARNING (IN BASSO) + ALGORITMO STESSO (IN ALTO)
...
# from task_envs import my_turtlex_wall # training environment
...
class SAC(object):
    def __init__(self, state_dim, action_dim, gamma=0.99,
                 tau=1e-2, alpha=0.2, hidden_dim=256, lr=0.0003):
    ...
    def save_models(self, episode_count):
        torch.save(self.policy.state_dict(), dirPath + '/my_sac/' + world + '/' + str(episode_count) + '_policy_net.pth')
        torch.save(self.critic.state_dict(), dirPath + '/my_sac/' + world + '/' + str(episode_count) + '_value_net.pth')
        # hard_update(self.critic_target, self.critic)
        # torch.save(soft_q_net.state_dict(), dirPath + '/my_sac/' + world + '/' + str(episode_count) + 'soft_q_net.pth')
        # torch.save(target_value_net.state_dict(), dirPath + '/my_sac/' + world + '/' + str(episode_count) + 'target_value_net.pth')
        print("====================================")
    ...
    def load_models(self, episode):
        self.policy.load_state_dict(torch.load(dirPath + '/my_sac/' + world + '/' + str(episode) + '_policy_net.pth'))
        self.critic.load_state_dict(torch.load(dirPath + '/my_sac/' + world + '/' + str(episode) + '_value_net.pth'))
        hard_update(self.critic_target, self.critic)
        # soft_q_net.load_state_dict(torch.load(dirPath + '/my_sac/' + world + '/'+str(episode) + 'soft_q_net.pth'))
        # target_value_net.load_state_dict(torch.load(dirPath + '/my_sac/' + world + '/'+str(episode) + 'target_value_net.pth'))
        print("====================================")
...
agent = SAC(state_dim, action_dim, gamma, tau, alpha, hidden_dim, learning_rate)
...
print('Action Minimums: ' + str(action_v_min) + ' m/s and ' + str(action_w_min) + ' rad/s')
print('Action Maximums: ' + str(action_v_max) + ' m/s and ' + str(action_w_max) + ' rad/s')



if __name__ == '__main__':

    rospy.init_node('turtlex_sac_office', anonymous=True, log_level=rospy.DEBUG) # change from rospy.WARN to rospy.DEBUG to view all the prints
    ...
    running_step = rospy.get_param("/turtlex/running_step")
    ...
    for ep in range(max_episodes):
        ...
        rospy.logdebug("############### WALL START EPISODE => " + str(ep))

        env.stats_recorder.done = None # CORRETTO DA ME; andava bene anche "env.stats_recorder.steps = 0", ma sarebbe stato meno corretto
        ...
        for step in range(max_steps):
            ...
            next_state, reward, done = env.step(unnorm_action, past_action) # TODO questo per ora e sbagliato: lo step deve essere di gym
                                                                            # devo stare attento qui e anche ai due punti in cui ho np.float32
            print('action', unnorm_action, 'r', reward)
            past_action = copy.deepcopy(action)

            rospy.logwarn("next_state: " + str(next_state) + " | reward: " + str(reward))
            ...
            state_joined = ''.join(map(str, state))
            rospy.logwarn("# state we were => " + str(state_joined))
            rospy.logwarn("# action that we took => " + str(action))
            rospy.logwarn("# reward that action gave => " + str(reward))
            rospy.logwarn("# episode cumulated_reward => " + str(cumulated_reward))
            next_state_joined = ''.join(map(str, next_state))
            rospy.logwarn("# State in which we will start next step => " + str(next_state_joined))
            ...
            rospy.logwarn("############### END Step => " + str(step))

        m, s = divmod(int(time.time() - start_time), 60)
        h, m = divmod(m, 60)
        rospy.logerr(("EP: " + str(ep + 1) + " | Reward: " + str(cumulated_reward) + " | Time: %d:%02d:%02d" % (h, m, s)))
    ...
    rospy.logwarn(("\n|" + str(max_episodes) + "|" + str(highest_reward)))
