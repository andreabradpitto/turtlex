...
class TurtlexArmTaskEnv(turtlex_arm_env.TurtlexArmEnv, utils.EzPickle):
    ...
    def __init__(self):
        ...
        #self.action_space = spaces.Box(self.joints_pos_min, self.joints_pos_max)
        ...
        self.observation_space = spaces.Box(low, high) # ee 3D position, current distance from goal = 4 observations
    ...
    def get_params(self):
        ...
        self.joints_pos_min = rospy.get_param("/turtlex_arm/joints_pos_min")
        self.joints_pos_max = rospy.get_param("/turtlex_arm/joints_pos_max")

        self.n_actions = rospy.get_param('/turtlex_arm/n_actions')
        self.n_observations = rospy.get_param('/turtlex_arm/n_observations')
        self.position_ee_max = rospy.get_param('/turtlex_arm/position_ee_max')
        self.position_ee_min = rospy.get_param('/turtlex_arm/position_ee_min')
        self.position_delta = rospy.get_param('/turtlex_arm/position_delta')
        ...
        self.init_pos = rospy.get_param('/turtlex_arm/init_pos')
        self.setup_ee_pos = rospy.get_param('/turtlex_arm/setup_ee_pos')
        self.goal_ee_pos = rospy.get_param('/turtlex_arm/goal_ee_pos')
    ...
    def _set_init_pose(self):
        ...
        rospy.logdebug("self.init_pos=" + str(self.init_pos))
        ...
        if self.movement_result:
            ...
            self.current_dist_from_des_pos_ee = self.compute_euclidean_dist(self.desired_position, self.last_gripper_target)
    ...
    def _init_env_variables(self):
        ...
        from a system that doesn't need the simulation to be running, like variables where the 
    ...
    def _set_action(self, action):
        ...
        if self.movement_result:
            # If the End Effector Positioning was succesful, we replace the last one with the new one
            self.last_gripper_target = copy.deepcopy(gripper_target)
        else:
            rospy.loginfo(tcolors.MAGENTA + "Impossible End Effector position: " + str(gripper_target) + tcolors.ENDC)
        
        rospy.loginfo(tcolors.CYAN + "END Set Action ==> " + str(action) + ", NAME=" + str(self.last_action) + tcolors.ENDC)
    ...
    def _get_obs(self):
        ...
        #rospy.logdebug("Observations ==> " + str(np.asarray(obs))) # TODO da testare
    ...
    def _compute_reward(self, observations, done):
        ...
        if self.movement_result:
            position_similar = np.all(np.isclose(self.desired_position, current_pos, atol=1e-02))

            rospy.loginfo(tcolors.CYAN + "desired_position = " + str(self.desired_position) + tcolors.ENDC)
            rospy.loginfo(tcolors.CYAN + "current_pos = " + str(current_pos) + tcolors.ENDC)
            rospy.loginfo(tcolors.CYAN + "self.current_dist_from_des_pos_ee = " + str(self.current_dist_from_des_pos_ee) + tcolors.ENDC)
            rospy.loginfo(tcolors.CYAN + "new_dist_from_des_pos_ee = " + str(new_dist_from_des_pos_ee) + tcolors.ENDC)
            
            delta_dist = new_dist_from_des_pos_ee - self.current_dist_from_des_pos_ee
            if position_similar:
                reward = self.reached_goal_reward
                rospy.loginfo(tcolors.CYAN + "Reached a desired position!" + tcolors.ENDC)
            else:
                if delta_dist < 0:
                    reward = self.closer_reward
                    rospy.loginfo(tcolors.CYAN + "Distance from desired position decreased! delta_dist = " + str(delta_dist) + tcolors.ENDC)
                else:
                    reward = self.step_punishment
                    rospy.loginfo(tcolors.CYAN + "Distance from desired position increased! delta_dist = " + str(delta_dist) + tcolors.ENDC)
            
        else:
            reward = self.impossible_movement_punishment
            rospy.loginfo(tcolors.CYAN + "Asked for an unreachable TCP position" + tcolors.ENDC)
        ...
        rospy.loginfo(tcolors.CYAN + ">>> REWARD >>> " + str(reward) + tcolors.ENDC)
    ...
    def calculate_if_done(self, movement_result,desired_position,current_pos):
        ...
        if movement_result:
            ...
            rospy.logdebug("Asked for an unreachable TCP position")
            
        return done

    def compute_euclidean_dist(self, v1, v2):
        """
        Calculates the Euclidean distance between two input vectors given as python lists
        """
