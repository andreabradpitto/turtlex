...
import fetch_env #from openai_ros.robot_envs import fetch_env_v2
...
class FetchReachEnv(fetch_env.FetchEnv, utils.EzPickle): # era fetch_env_v2.FetchEnv
    ...
    def __init__(self):
        ...
        fetch_env.FetchEnv.__init__(self) # era fetch_env_v2.FetchEnv
        utils.EzPickle.__init__(self)
        
        rospy.logdebug("Call env setup")
        self._env_setup(initial_qpos=self.init_pos)
        
        rospy.logdebug("Call get_obs")
        obs = self._get_obs()
    ...
    def _get_obs(self):
        ...
        #rospy.logdebug(grip_rpy)
        grip_velp = np.array([grip_rpy.y, grip_rpy.y])
        robot_qpos, robot_qvel = self.robot_get_obs(self.joints)
        if self.has_object:
            object_pos = self.sim.data.get_site_xpos('object0')
            # rotations
            object_rot = self.mat2euler(self.sim.data.get_site_xmat('object0')) # rotations.mat2euler(self.sim.data.get_site_xmat('object0'))
            # velocities
