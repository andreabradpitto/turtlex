...
class SAC(object):
    def __init__(self, state_dim, action_dim, gamma=0.99, tau=1e-2, alpha=0.2, hidden_dim=30, lr=0.0003):
    ...
    def load_models(self, netspath, net_id):
        self.policy.load_state_dict(torch.load(netspath + net_id + ".pth", map_location=self.device))
