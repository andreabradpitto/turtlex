...
class DropToy(tree.Node):
    ...
    def __init__(self, name, counter, *args, **kwargs):
        super(DropToy, self).__init__(
            name=name, configure_cb=self.configure, run_cb=self.run, *args, **kwargs)
        rospy.loginfo("setting up DropToy for " + self._name)
        self.counter = counter

    def configure(self, nodedata):
        rospy.logdebug("DropToy.configure()" + self._name)
        self.counter = nodedata.get_data('counter', self.counter)

    def run(self, nodedata):
        rospy.logdebug("DropToy.run()" + self._name)
        if self.counter > 0:
            self.counter -= 1
        else:
